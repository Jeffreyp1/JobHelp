/**
 * Mirror of extension-app/extension/src/types/api-contract.ts — kept in sync manually.
 * Apps Script's V8 runtime can read TS files via clasp's tsc compilation.
 *
 * SOURCE OF TRUTH: extension-app/extension/src/types/api-contract.ts. When that file changes,
...
export interface SeedDefaultsRequest {
  action: "seed_defaults";
  rulesFolderId: string;
  /** Base raw URL for fetching rule files; e.g. https://raw.githubusercontent.com/<user>/<repo>/main/extension-app/prompts/shared */
  rawBaseUrl: string;
