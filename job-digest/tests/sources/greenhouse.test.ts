...
describe('greenhouse adapter', () => {
  ...
  describe('fetch()', () => {
    ...
    it('throws SourceFetchError with type=rate_limit on 429', async () => {
      ...
      vi.stubGlobal('fetch', fetchMock);
      // The adapter retries 429 thrice with 2s/4s/6s backoff before giving up.
      await expect(greenhouse.fetch(makeConfig(['acmecorp']))).rejects.toMatchObject({ type: 'rate_limit' });
    }, 20000);

    it('throws SourceFetchError with type=not_found on 404 (board not found)', async () => {
      const fetchMock = vi.fn().mockImplementation(() => Promise.resolve(
        new Response('not found', { status: 404 }),
      ));
      vi.stubGlobal('fetch', fetchMock);
      await expect(greenhouse.fetch(makeConfig(['ghost']))).rejects.toMatchObject({ type: 'not_found' });
    });
    ...
    it('throws when ALL tokens fail', async () => {
      ...
      vi.stubGlobal('fetch', fetchMock);
      await expect(greenhouse.fetch(makeConfig(['a', 'b']))).rejects.toMatchObject({ type: 'server' });
    });
  });
});
