# JobHelp MCP Maintainer Notes

This document describes the MCP server implementation inside `jobhelp-mcp/`.
For user install snippets, example sessions, and the package-level tool table,
prefer `jobhelp-mcp/README.md`; this file is for maintainers who need to change
or debug the server.
...
- `jobhelp-mcp/mcp/src/bin.ts` is the executable entrypoint. It loads package
  metadata, bootstraps dependencies, builds the MCP server, and connects stdio.
- `jobhelp-mcp/mcp/src/index.ts` builds the MCP `Server`, registers request
  handlers for tools, resources, and prompts, and maps unknown names to typed
  errors where the MCP SDK allows it.
- `jobhelp-mcp/mcp/src/wiring.ts` connects the MCP layer to core operations.
  It builds `CoreDeps` for tools and `ResourceDeps` for resources.
- `jobhelp-mcp/mcp/src/wiring-cache.ts` provides lazy dependency resolution so
  a server started before first-run config exists can still serve `init_config`
  and recover after config is written.
- `jobhelp-mcp/core/` owns durable behavior: source adapters, config loading,
  digest generation, ranking pipeline, resume registry, application output,
  rules, and state.

The package build compiles both MCP and the core code into `jobhelp-mcp/mcp/dist/`.
...
Run these from `jobhelp-mcp/` unless a broader repo check is needed:
...
- Keep `jobhelp-mcp/README.md` as the user-facing install and usage source.
...
- When adding a tool, update the relevant `tools-*.ts` module, parser, types,
  tests under `jobhelp-mcp/tests/mcp/`, and the README tool table if user-facing
  behavior changes.
...
Run `npm run build` from `jobhelp-mcp/` and verify that `mcp/dist/mcp/src/bin.js`
