...
- v2 actions are called **directly from the side panel** via [`apiClient`](../extension-app/extension/src/lib/apiClient.ts) — they do not pass through the background service worker. The background worker still owns `generate` / `finalize` / `list_files` / `seed_defaults` / `write_file` / `download_template` / `upload_filled_docx`.
