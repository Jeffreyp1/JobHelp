// extension/public/data/skills-dict.json
var skills_dict_default = {
  version: "1.0",
  source: "bundled",
  totalCanonical: 608,
  totalLookups: 4015,
  skills: [
    ...
    {
      canonical: "Progressive Enhancement",
      synonyms: [
    ...
    {
      canonical: "Azure Active Directory",
      synonyms: [
        "azure ad",
    ...
    },
    {
      ...
      synonyms: [
        ...
        "google gke",
        "managed kubernetes gcp"
      ]
    },
    {
      canonical: "Google Cloud Storage",
      synonyms: [
    ...
    {
      canonical: "Google Cloud Pub/Sub",
      synonyms: [
        "pub/sub",
        "pubsub",
        "google pubsub",
    ...
    {
      canonical: "Databricks Delta Lake",
      synonyms: [
        "delta lake",
        "databricks delta",
        "delta tables",
    ...
    {
      canonical: "Continuous Improvement",
      synonyms: [
    ...
    {
      canonical: "Technical Program Manager",
      synonyms: [
    ...
    {
      canonical: "Open Source Developer",
      synonyms: [
        "open source",
    ...
    {
      canonical: "Low-level Programming",
      synonyms: [
        "low level",
        "systems programming",
        "memory management",
    ...
    {
      canonical: "AWS Elastic Beanstalk",
      synonyms: [
        "elastic beanstalk",
        "beanstalk",
        "aws beanstalk",
    ...
    {
      canonical: "Azure Container Apps",
      synonyms: [
    ...
    {
      canonical: "Azure Cache for Redis",
      synonyms: [
        "azure redis",
        "azure cache",
    ...
    {
      canonical: "Google Cloud Spanner",
      synonyms: [
        "cloud spanner",
        "spanner",
        "google spanner",
    ...
    {
      canonical: "Google Cloud Firestore",
      synonyms: [
        "firestore",
        "cloud firestore",
        "google firestore",
    ...
    {
      canonical: "Google Cloud Bigtable",
      synonyms: [
        "cloud bigtable",
        "gcp bigtable",
        "google bigtable"
    ...
    {
      canonical: "Cloudflare Zero Trust",
      synonyms: [
    ...
    {
      canonical: "Triton Inference Server",
      synonyms: [
        "triton",
        "triton inference",
        "nvidia triton",
    ...
    {
      canonical: "Observability Engineer",
      synonyms: [
    ...
    {
      canonical: "Cross-platform Developer",
      synonyms: [
        "cross-platform developer",
    ...
    {
      canonical: "Data Platform Engineer",
      synonyms: [
...
// extension/src/lib/skillsDict.ts
async function loadSkillsDict() {
  return buildDictMap(skills_dict_default);
}
function buildDictMap(data) {
...
var TOKEN_RE = /[\s,.;!?()\[\]{}'"/\\|@#$%^&*+=<>~`]+/;
function findSkillsInText(text, dict) {
...
var _multiWordFlagCache = /* @__PURE__ */ new WeakMap();
function _getMultiWordFlag(dict) {
  ...
  for (const s of Array.from(scripts)) {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        for (const p of parsed) if (p && typeof p === "object") out.push(p);
...
function metaContent(doc, key, attr = "property") {
  const el = doc.querySelector(`meta[${attr}="${key}"]`);
  return el?.content?.trim() || null;
}
function parseTitleForRoleAndCompany(doc) {
...
function parseSalary(text) {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (minVal < 1e4 && (maxVal === null || maxVal < 1e4)) continue;
    bestMin = minVal;
...
function parseSalaryNumber(s, kSuffix) {
  const cleaned = s.replace(/,/g, "");
  const n = parseInt(cleaned, 10);
  if (!Number.isFinite(n)) return null;
  if (kSuffix && n < 1e4) return n * 1e3;
  return n;
}
...
function parseLocationAndRemote(text, doc, header) {
  const ld = findJobPosting(doc);
  let ldLocation = null;
  if (ld) {
  ...
  if (!location2 && ldLocation) location2 = ldLocation;
  if (!location2 && (isRemote || isHybrid)) {
    location2 = isRemote ? "Remote" : "Hybrid";
  }
  if (!remote && location2 && /^remote/i.test(location2)) {
    remote = "remote";
  }
  if (!remote && location2) {
    remote = "onsite";
  }
  return { location: location2, remote };
}
...
function parseVisa(text) {
  if (/\b(?:not?\s+(?:able|eligible)\s+to\s+sponsor|unable\s+to\s+sponsor|do\s+not\s+(?:offer|provide)\s+sponsor|cannot\s+sponsor|will\s+not\s+sponsor)\b/i.test(text)) {
    return "no";
  }
...
function parsePostedDate(doc) {
  ...
  if (ld && typeof ld["datePosted"] === "string") {
    return normalizeDate(ld["datePosted"]);
  }
  return null;
}
...
// extension/scripts/_scraper-shim.ts
window.__jobhelpScrape = () => scrapePage({ document, url: location.href });
//# sourceMappingURL=scraper.bundle.js.map
