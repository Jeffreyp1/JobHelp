...
describe('bm25 — buildCorpus', () => {
  it('empty corpus scores 0 with no NaN', () => {
    ...
    const s = scoreBM25F(corpus, 0, ['kafka'], DEFAULT_BM25_PARAMS);
    expect(s).toBe(0);
...
describe('bm25 — scoreBM25F', () => {
  ...
  it('term in title weighted more than same term in description', () => {
    ...
    const sTitle = scoreBM25F(corpus, 0, ['kafka'], DEFAULT_BM25_PARAMS);
    const sDesc = scoreBM25F(corpus, 1, ['kafka'], DEFAULT_BM25_PARAMS);
  ...
  it('TF saturation: 10× same term in body is NOT 10× score', () => {
    ...
    const corpus = buildCorpus(jobs, tok);
    const s10 = scoreBM25F(corpus, 0, ['kafka'], DEFAULT_BM25_PARAMS);
    const s2 = scoreBM25F(corpus, 1, ['kafka'], DEFAULT_BM25_PARAMS);
  ...
  it('length norm: same term in short desc > same term in long desc', () => {
    ...
    const corpus = buildCorpus(jobs, tok);
    const sShort = scoreBM25F(corpus, 0, ['kafka'], DEFAULT_BM25_PARAMS);
    const sLong = scoreBM25F(corpus, 1, ['kafka'], DEFAULT_BM25_PARAMS);
  ...
  it('field weights are configurable via params arg', () => {
    ...
    const sHeavy = scoreBM25F(corpus, 0, ['kafka'], heavy);
    const sLight = scoreBM25F(corpus, 0, ['kafka'], light);
  ...
  it('multi-term query scores sum across terms', () => {
    ...
    const corpus = buildCorpus(jobs, tok);
    const sBoth = scoreBM25F(corpus, 0, ['kafka', 'redis'], DEFAULT_BM25_PARAMS);
    const sKafka = scoreBM25F(corpus, 0, ['kafka'], DEFAULT_BM25_PARAMS);
    const sRedis = scoreBM25F(corpus, 0, ['redis'], DEFAULT_BM25_PARAMS);
  ...
  it('IDF floor: very common term (df ≈ N) still scores > 0', () => {
    ...
    const corpus = buildCorpus(jobs, tok);
    const s = scoreBM25F(corpus, 0, ['engineer'], DEFAULT_BM25_PARAMS);
    expect(s).toBeGreaterThan(0);
  });

  it('empty query returns 0', () => {
    const corpus = buildCorpus([doc({ description: 'kafka' })], tok);
    const s = scoreBM25F(corpus, 0, [], DEFAULT_BM25_PARAMS);
    expect(s).toBe(0);
  });
  ...
  it('IDF applied once per query term: multi-field match score stays within saturation ceiling', () => {
    ...
    const scoreA = scoreBM25F(corpus, 0, ['kafka'], DEFAULT_BM25_PARAMS);
  ...
  it('idf returns the floor when df exceeds N (mismatched-corpus defense)', () => {
    ...
    const s = scoreBM25F(stale, 0, ['kafka'], DEFAULT_BM25_PARAMS);
    expect(Number.isFinite(s)).toBe(true);
    expect(Number.isNaN(s)).toBe(false);
    expect(s).toBeGreaterThan(0);
  });
